...
class Actor(base_actor.BaseActor):
    ...
    def _build_network(self):
        ...
        self.new_trainable_variable("w_sin", np.zeros(
            (config.somites * 2 - 1, config.oscillators), dtype=np.float64))
    ...
    def _calc_fs(self, state: np.array) -> tuple:
        assert state.shape == (config.somites * 2 - 1,), "state shape should be {}, got {}".format((config.somites * 2 - 2,), state.shape)
