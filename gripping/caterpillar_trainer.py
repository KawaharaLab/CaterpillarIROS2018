...
def new_caterpillar() -> Caterpillar:
    return Caterpillar(config.somites, config.oscillators_list, config.grippers_list, config.caterpillar_params)
...
def test_current_params(actor: base_actor, log_dir: str, episode: int):
    ...
    sim_gripping_phase_diffs_file = DataCSVSaver(
        "{}/train_result_ep{}_gripping_phase_diffs.txt".format(log_dir, episode),
    ...
    for step in range(int(steps)):
        try:
            ...
            sim_gripping_phase_diffs_file.append_data(step, *utils.mod2pi(phases[config.oscillators:]))
            sim_somite_phases_file.append_data(step, *utils.mod2pi(phases[:config.oscillators]))
            sim_gripping_phases_file.append_data(step, *utils.mod2pi(phases[config.oscillators:]))
