...
base_dir = os.path.join("/Users/luning/ResearchMaster/src/iros/gripping/results/muscle/normal/k40/run0")
    ...
    for s_id, pos in frames[keyframe]:
        if s_id == '_somite_0':
            for bone_id in somites_bones[s_id]:
                ...
                cmds.setKeyframe('Bone{0:03d}'.format(bone_id), t=keyframe ,v=4*pos[2], at='translateY')
        else:
            for bone_id in somites_bones[s_id]:
                ...
                cmds.setKeyframe('Bone{0:03d}'.format(bone_id), t=keyframe ,v=4*(pos[2] - prev_somite_pos[2]), at='translateY')
