...
def observe_and_act(actor: BaseActor, caterpillar: Caterpillar, episode=None, disable_list=[], broken_value=0):
    """
        To save and visualize variables in Tensorboard, you should provide episode.
    """
    assert np.all(np.array(disable_list) < config.params["somites"])
    mask = np.ones(config.params["somites"])
    mask[disable_list] = 0
    bias = np.zeros(config.params["somites"])
...
def run_caterpillar(actor, save_dir: str, steps: int, disable_list=[], broken_value=0):
    ...
    sim_phase_diffs_file = DataCSVSaver(os.path.join(save_dir, "phase_diffs.txt"), ["step"] + ["phi_{}".format(i) for i in range(config.params["oscillators"])])
    sim_phases_file = DataCSVSaver(os.path.join(save_dir, "phases.txt"), ["step"] + ["phi_{}".format(i) for i in range(config.params["oscillators"])])
    sim_actions_file = DataCSVSaver(os.path.join(save_dir, "actions.txt"), ["step"] + ["action_{}".format(i) for i in range(config.params["oscillators"])])
    sim_frictions_file = DataCSVSaver(os.path.join(save_dir, "frictions.txt"), ["step"] + ["friction_{}".format(i) for i in range(config.params["somites"])])
...
if __name__ == '__main__':
    ...
    parser.add_option("-s", "--steps", default=10000, dest="steps", type="int")
    ...
    actor_class = getattr(module_, opts.actor_name)
